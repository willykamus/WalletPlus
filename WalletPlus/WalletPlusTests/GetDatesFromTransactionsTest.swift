//
//  GetDatesFromTransactionsTest.swift
//  WalletPlusTests
//
...
class GetDatesFromTransactionsTest: XCTestCase {
    ...
    func testGetDatesFromTransactions() {
        //Given
        let data: [Transaction] = data.transactions
        let getDates: GetDates = GetDatesInteractor()
        
        // When
        let dates: [Date] = getDates.getDates(from: data)
        
        // Then
        XCTAssertTrue(dates.count > 0)
        XCTAssertTrue(dates.count == 2)
        XCTAssertEqual(dates[1], Date(timeIntervalSinceReferenceDate: 1000000))
        XCTAssertEqual(dates[0], Date(timeIntervalSinceReferenceDate: 40000000))
    }
